...
log_file_NL_50000="/home/ubuntu/google-patent-scraper/output_NL_50000.log"
...
if pgrep -f "$script_name --country_code NL --from_index 30000 --to_index 40000" > /dev/null; then
    ...
    nohup python -u copy_google_patent.py --country_code NL --from_index 30000 --to_index 40000 > "$log_file_NL_30000" 2>&1 &
fi

if pgrep -f "$script_name --country_code NL --from_index 40000 --to_index 50000" > /dev/null; then
    echo "Another instance of the script with country code NL is already running. Exiting."
else
    nohup python -u copy_google_patent.py --country_code NL --from_index 40000 --to_index 50000 > "$log_file_NL_40000" 2>&1 &
fi

if pgrep -f "$script_name --country_code NL --from_index 50000 --to_index 60000" > /dev/null; then
    echo "Another instance of the script with country code NL is already running. Exiting."
else
    nohup python -u copy_google_patent.py --country_code NL --from_index 50000 --to_index 60000 > "$log_file_NL_50000" 2>&1 &
fi